...
- Every Apple `buildUploads` attempt, including failed attempts, consumes its number. Unknown or in-flight upload state fails closed. Allocation is reread just before the only upload attempt; the existing PEAR distribution workflow concurrency group serializes runs and never cancels an uploader. The archive export runs with `manageAppVersionAndBuildNumber` off and `destination` `export`; otherwise Xcode 26 registers an empty `AWAITING_UPLOAD` record during export, and a run that fails after export leaves it behind. Reconcile such a leftover (no files, no build) with Apple's documented `DELETE /v1/buildUploads/{id}` readback-first, never by weakening the guard.
