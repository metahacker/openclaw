...
- Every Apple `buildUploads` attempt, including failed attempts, consumes its number. Unknown or in-flight upload state fails closed. Allocation is reread just before the only upload attempt; the existing PEAR distribution workflow concurrency group serializes runs and never cancels an uploader. The archive export runs with `manageAppVersionAndBuildNumber` off and `destination` `export`, yet Xcode 26 still registers an empty `AWAITING_UPLOAD` placeholder for the exported build number during export. The lane removes only the placeholder it provably created (new since allocation, this version and build, no files, no build) with Apple's documented `DELETE /v1/buildUploads/{id}` before the pre-upload recheck; a run that dies between export and that step leaves the placeholder behind, and the next release refuses until it is reconciled the same way, readback-first, never by weakening the guard.
