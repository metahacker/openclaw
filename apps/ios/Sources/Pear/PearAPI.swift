...
struct PearHomeFeed: Codable {
    struct Item: Codable {
        ...
        var bestTag: String? {
            self.tag ?? self.hashtag ?? self.projectTag
        }
        ...
        var bestURL: String? {
            self.url ?? self.href ?? self.path
        }

        var bestEmoji: String? {
            self.emoji ?? self.projectEmoji
        }

        var isPinnedInFeed: Bool {
            self.pinned ?? self.isPinned ?? false
        }
    }
    ...
    struct Motion: Codable {
        ...
        var bestStatus: String? {
            self.status ?? self.detail ?? self.whereItStands
        }
...
struct PearConversationSummary: Codable, Identifiable {
    ...
    var bestTag: String? {
        self.projectTag ?? self.sourceTag
    }
}
